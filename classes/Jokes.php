...
public $COMMENT;
    ...
    <a href='/view/".$row['ID']."/$sname.html' class='pull-left thumb-sm m-r'> <img src='/image/$image' alt='...'> </a>
    <a class='clear' href='/view/".$row['ID']."/$sname.html'> <span class='block text-ellipsis'>".$row['HEADLINE']."</span> <small class='text-muted'>by </small> </a>
        </li>";
        }
        echo "</ul>
</section>";
    ...
    echo "<a class='font-semibold' href='/view/".$row['ID']."/$sname.html'>".$row['HEADLINE']."</a></div> </article>";
        }
    }
    
    public function RelatedPOsts($type, $id){
       $Q = "SELECT * FROM JOKES WHERE TYPE = $type AND ID <> $id ORDER BY ID DESC LIMIT 5";
       $result = null;
       try{
        $stmt = $this->conn->query($Q);
        $stmt->execute();
    ...
    echo "<li class='list-group-item clearfix bg bg-succss'>
    <a href='/view/".$row['ID']."/$sname.html' class='pull-left thumb-sm m-r'> <img src='/image/$image' alt='...'> </a>
    <a class='clear' href='/view/".$row['ID']."/$sname.html'> <span class='block text-ellipsis'>".$row['HEADLINE']."</span>"
    ...
    public function TopFiveJoke($type){
       $Q = "SELECT * FROM JOKES WHERE TYPE = $type ORDER BY ID DESC LIMIT 5";
       $result = null;
       try{
        $stmt = $this->conn->query($Q);
        $stmt->execute();
     ...
     echo "<a href='/view/".$row['ID']."/$sname.html' class='list-group-item clearfix'>";
                echo "<span class='pull-left thumb-sm avatar m-r  hidden-xs'> "
     . "<img src='/image/".$image."' alt='...'></span>";
       echo " <span class='clear'> <span>".$row['HEADLINE']."</span>";
       echo "<small class='text-muted clear text-ellipsis'>Assamese Jokes</small> </span> </a>";
...
public function setCATEGORY($STATUS){$this->CATEGORY=$STATUS;}
