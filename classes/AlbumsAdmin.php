...
class Albums {
    ...
    $stmt->bindParam(5, $this->ART_LINK);
    ...
    $stmt->bindParam(15, $this->CREATE_ON);
    $stmt->execute();
...
return $success;}
    ...
    $query = "UPDATE ALBUM SET LANGUAGE_ID = ?, ALBUM_NAME = ?, PRODUCTION = ?, YEAR = ?, DESCRIPTION = ?, "
            . "CATEGORY_ID = ?, MUSIC = ?, COVER = ?, NEWOLD = ?, STATUS = ?, BITRATE = ?, LABEL = ? WHERE ID = ?";
    $success = 0;
    try{
    $stmt = $this->conn->prepare($query);
    $stmt->bindParam(1, $this->LANGUAGE_ID);
    ...
    $query = "SELECT * FROM ALBUM WHERE ALBUM_NAME =?";
    $success = 0;
try{
    $stmt = $this->conn->prepare($query);
...
return $success;}
    ...
    public function UpdateAlbumSingle($colm, $id, $getValue){
        $query = "UPDATE ALBUM SET $colm = ? WHERE ID = ? ";
        $success = 0;
        try {
            $stmt = $this->conn->prepare($query);
    ...
    public function RandomAlbum($start, $limit){
       $Q = "SELECT * FROM ALBUM ORDER BY ID DESC LIMIT ".$start." ," . $limit ." ";
       $result = null;
       try{
        $stmt = $this->conn->query($Q);
        $stmt->execute();
    ...
    public function AlbumByCat($start, $limit, $cid){
       $Q = "SELECT * FROM ALBUM WHERE CATEGORY_ID = $cid ORDER BY ID DESC LIMIT ".$start." ," . $limit ." ";
       $result = null;
       try{
        $stmt = $this->conn->query($Q);
        $stmt->execute();
    ...
    public function RelatedAlbums($id){
       $Q = "SELECT * FROM ALBUM WHERE ID != $id ORDER BY ID DESC LIMIT 10";
       $result = null;
       try{
        $stmt = $this->conn->query($Q);
        $stmt->execute();
    ...
    <a href='files.php?i=".$row['ID']."' class='pull-left thumb-sm m-r'> <img src='$image' alt='...'> </a>
    <a class='clear' href='files.php?i=".$row['ID']."'> <span class='block text-ellipsis'>".$row['ALBUM_NAME']."</span> <small class='text-muted'>by $art</small> </a>
        </li>";
        }
        echo "</ul>
</section>";
       return $result;
        ...
        public function ArtistByAlbumList($id){
    ...
    private $COVER;
    private $DESCRIPTION;
    private $NEWOLD; // 1 = old || 0 = New || 2 = Upcomming
