...
class Category {
    ...
    $query = "INSERT INTO CATEGORY(LANGUAGE_ID, CATEGORY_NAME, DESCRIPTION, IMAGE_LINK, TYPE) VALUES (?, ?, ?, ?, ?)";
    $success = 0;
    try{
    ...
    $query = "UPDATE CATEGORY SET LANGUAGE_ID = ?, CATEGORY_NAME =?, DESCRIPTION =?, TYPE =? WHERE ID = ?";
    $success = 0;
    try{
    $stmt = $this->conn->prepare($query);
    $stmt->bindParam(1, $this->LANGUAGE_ID);
...
return $success;}
    ...
    $Q = "SELECT COUNT(*) AS TOTAL FROM CATEGORY WHERE TYPE = $id";
    $total = 0;
    try{
    ...
    return $total;}
       ...
       $Q = "SELECT * FROM CATEGORY WHERE TYPE = $type ORDER BY ID DESC LIMIT 30";
       $result = null;
       try{
       $stmt = $this->conn->query($Q);
       $stmt->execute();
    ...
    public function LoadCategoryDropDown($type){
        include('../include/database.php');
        mysqli_select_db($conn, $data);
        $view = "SELECT * FROM CATEGORY WHERE TYPE = $type ORDER BY ID DESC LIMIT 30";
        $rs = mysqli_query($conn, $view) or die(mysqli_error($conn));
        $rows = mysqli_fetch_assoc($rs);
        if($rows > 0)
            {
            do{
                echo "<option class='form-control' value='".$rows["ID"]."'>".$rows['CATEGORY_NAME']."</option>";
            }
        while($rows=mysqli_fetch_assoc($rs));   
    ...
    {   do{
            ...
            $songs = $this->CountSongs($row['ID']);
            echo "<a href='scat.php?i=".$row['ID']."' class='list-group-item'>".$row['CATEGORY_NAME']." <span class='pull-right badge bg-info'>$songs</span></a>";
            }
...
private $DESCRIPTION;
private $IMAGE_LINK;
private $TYPE; // 0 = songs | 1 = Videos || 2 SMS || 3 ==Blogs || 4 == Photos
private $conn;
